...
#define MESSAGE_TOPIC_START 1
#define MESSAGE_TOPIC_ENABLE 2
#define MESSAGE_TOPIC_DISABLE 3
#define MESSAGE_TOPIC_STANDBY 4
#define MESSAGE_TOPIC_APPLIED_SETTINGS_MATCH_START 5
#define MESSAGE_TOPIC_ERROR_CODE 6
#define MESSAGE_TOPIC_SETTING_VERSIONS 7
#define MESSAGE_TOPIC_SUMMARY_STATE 8
#define MESSAGE_TOPIC_DETAILED_STATE 9
#define MESSAGE_TOPIC_EXIT 10
...
void processLogEventSettingVersionsClientRequest() {
    ...
    if (enabledTopics[MESSAGE_TOPIC_SETTING_VERSIONS]) {
        bufferIndex = 0;
        std::string* recommendedSettingVersion = getString(buffer, &bufferIndex);
        eventSettingVerionsData.recommendedSettingVersion = *recommendedSettingVersion;
        ...
        delete recommendedSettingVersion;
    }
}
...
void processLogEventDetailedStateClientRequest() {
    ...
    if (enabledTopics[MESSAGE_TOPIC_DETAILED_STATE]) {
        bufferIndex = 0;
...
void processLogEventClientRequest() {
    ...
    switch(messageHeader.topic) {
        ...
        case MESSAGE_TOPIC_DETAILED_STATE: processLogEventDetailedStateClientRequest(); break;
        default:
            printf("Unknown message header topic %d\n", messageHeader.topic);
            break;
    }
...
void processIssueCommandStartClientRequest() {
    ...
    if (enabledTopics[MESSAGE_TOPIC_START]) {
        bufferIndex = 0;
        std::string* configuration = getString(buffer, &bufferIndex);
        commandStartData.configuration = *configuration;
        commandId = m1m3SAL.issueCommand_start(&commandStartData);
        commandStartData.configuration = "";
...
void processIssueCommandExitClientRequest() {
    ...
    if (enabledTopics[MESSAGE_TOPIC_EXIT]) {
        bufferIndex = 0;
        commandExitData.exitIsTrue = getU8(buffer, &bufferIndex);
...
void processGetEventSettingVerionsSALRequest() {
    if (enabledTopics[MESSAGE_TOPIC_SETTING_VERSIONS]) {
        ...
        if (rc == SAL__OK) {
            ...
            bufferIndex = 0;
            setString(buffer, &bufferIndex, &eventSettingVerionsData.recommendedSettingVersion);
...
void processGetEventDetailedStateSALRequest() {
    if (enabledTopics[MESSAGE_TOPIC_DETAILED_STATE]) {
        ...
        if (rc == SAL__OK) {
            ...
            writeClientHeader(MESSAGE_TYPE_GET_EVENT, MESSAGE_TOPIC_DETAILED_STATE, bufferIndex);
            writeBuffer(buffer, bufferIndex);
        }
    }
}
...
void processAcceptCommandStartSALRequest() {
    if (enabledTopics[MESSAGE_TOPIC_START]) {
        ...
        if (commandId > 0) {
            ...
            bufferIndex = 0;
            setI32(buffer, &bufferIndex, commandId);
            setString(buffer, &bufferIndex, &commandStartData.configuration);
...
void processAcceptCommandExitSALRequest() {
    if (enabledTopics[MESSAGE_TOPIC_EXIT]) {
        ...
        if (commandId > 0) {
            ...
            bufferIndex = 0;
            setI32(buffer, &bufferIndex, commandId);
            setU8(buffer, &bufferIndex, commandExitData.exitIsTrue);
...
int main(int argumentCount, char *argumentValues[]) {
    ...
    if (enabledTopics[MESSAGE_TOPIC_EXIT]) {
        m1m3SAL.salCommand("m1m3_command_Exit");
        m1m3SAL.salProcessor("m1m3_command_Exit");
    }
    ...
    if (enabledTopics[MESSAGE_TOPIC_DETAILED_STATE]) {
        m1m3SAL.salEvent("m1m3_logevent_DetailedState");
    }
