...
void processArgs(int argc, char *const argv[], const char *&configRoot) {
  ...
  while ((opt = getopt(argc, argv, "bc:dfhp:sSu:vV")) != -1) {
    ...
    case 'v':
      ...
      exit(EXIT_SUCCESS);
    case 'V':
      std::cout << "SAL " << SAL_MTM1M3::getSALVersion() << " "
                << SAL_MTMount::getSALVersion() << std::endl
                << "XML " << SAL_MTM1M3::getXMLVersion() << " "
                << SAL_MTMount::getXMLVersion() << std::endl
                << "OSPL " << SAL_MTM1M3::getOSPLVersion() << " "
                << SAL_MTMount::getOSPLVersion() << std::endl;
      exit(EXIT_SUCCESS);
    default:
...
void runFPGAs(std::shared_ptr<SAL_MTM1M3> m1m3SAL,
              std::shared_ptr<SAL_MTMount> mtMountSAL, int retPipe) {
  ...
  try {
    ...
    std::thread outerLoopClock(
        [&outerLoopClockThread] { outerLoopClockThread.run(); });
    ...
    SPDLOG_CRITICAL("Error starting.stopping or joining threads: {)",
                    ex.what());
  }
}
...
void setSinks() {
  auto logger = std::make_shared<spdlog::async_logger>(
      "M1M3support", sinks.begin(), sinks.end(), spdlog::thread_pool(),
...
void startLog() {
  ...
  if (enabledSinks & 0x04) {
    auto syslog_sink = std::make_shared<spdlog::sinks::syslog_sink_mt>(
        "M1M3support", LOG_PID | LOG_CONS, LOG_USER, false);
...
int main(int argc, char *const argv[]) {
  ...
  if (pidFile) {
    ...
    if (pipe(startPipe) == -1) {
      std::cerr << "Error: Cannot create pipe for child/start process: "
    ...
    if (child > 0) {
      ...
      pidf.close();
      if (pidf.fail()) {
        std::cerr << "Error: Cannot write to PID file " << pidFile << ": "
                  << strerror(errno) << std::endl;
        exit(EXIT_FAILURE);
      }
      if (chown(pidFile, runAs->pw_uid, runGroup->gr_gid)) {
        std::cerr << "Error: Cannot change owner of " << pidFile << ":"
                  << strerror(errno) << std::endl;
        exit(EXIT_FAILURE);
      }
      char retbuf[2000];
      ...
      signal(SIGALRM, [](int) {
        std::cerr << "Error: Start timeouted, see syslog for details."
                  << std::endl;
        exit(EXIT_FAILURE);
      });
      alarm(30);
  ...
  if (SAL_MTM1M3::getSALVersion() != SAL_MTMount::getSALVersion()) {
    SPDLOG_WARN("SAL version mismatch: MTM1M3 {} MTMount {}",
                SAL_MTM1M3::getSALVersion(), SAL_MTMount::getSALVersion());
  }
  if (SAL_MTM1M3::getXMLVersion() != SAL_MTMount::getXMLVersion()) {
    SPDLOG_WARN("XML mismatch: MTM1M3 {} MTMount {}",
                SAL_MTM1M3::getXMLVersion(), SAL_MTMount::getXMLVersion());
  }
  if (SAL_MTM1M3::getOSPLVersion() != SAL_MTMount::getOSPLVersion()) {
    SPDLOG_WARN("OSPL version mismatch: MTM1M3 {} MTMount {}",
                SAL_MTM1M3::getOSPLVersion(), SAL_MTMount::getOSPLVersion());
  }
