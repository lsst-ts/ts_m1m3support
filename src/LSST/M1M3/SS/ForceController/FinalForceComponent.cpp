...
void FinalForceComponent::applyForcesByComponents() {
    ...
    for (int i = 0; i < 156; ++i) {
        if (i < 12) {
            this->xTarget[i] =
                    (_appliedAccelerationForces->xForces[i] + _appliedAzimuthForces->xForces[i] +
        ...
        if (i < 100) {
            this->yTarget[i] =
                    (_appliedAccelerationForces->yForces[i] + _appliedAzimuthForces->yForces[i] +
        ...
        this->zTarget[i] =
                (_appliedAberrationForces->zForces[i] + _appliedAccelerationForces->zForces[i] +
...
void FinalForceComponent::postUpdateActions() {
    ...
    for (int zIndex = 0; zIndex < 156; ++zIndex) {
        ...
        if (xIndex != -1) {
            ...
            notInRange =
                    !Range::InRangeAndCoerce(xLowFault, xHighFault, _rejectedForces->xForces[xIndex],
        ...
        if (yIndex != -1) {
            ...
            notInRange =
                    !Range::InRangeAndCoerce(yLowFault, yHighFault, _rejectedForces->yForces[yIndex],
    ...
    fm = ForceConverter::calculateForcesAndMoments(
            ...
            _rejectedForces->xForces, _rejectedForces->yForces, _rejectedForces->zForces);
