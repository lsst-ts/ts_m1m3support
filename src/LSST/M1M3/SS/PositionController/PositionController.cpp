...
bool PositionController::forcesInTolerance() {
    ...
    for (int i = 0; i < HP_COUNT; i++) {
        inTolerance = inTolerance &&
                      Range::InRange((float)this->positionControllerSettings->RaiseLowerForceLimitLow,
                                     (float)this->positionControllerSettings->RaiseLowerForceLimitHigh,
                                     this->hardpointActuatorData->measuredForce[i]);
    }
    return inTolerance;
}
...
bool PositionController::move(int32_t* steps) {
    ...
    for (int i = 0; i < HP_COUNT; i++) {
        ...
        this->hardpointActuatorState->motionState[i] = steps[i] != 0 ? HardpointActuatorMotionStates::Stepping
    ...
    for (int i = 0; i < HP_COUNT; ++i) {
        this->scaledMaxStepsPerLoop[i] = (int32_t)(this->positionControllerSettings->MaxStepsPerLoop /
                                                   (maxLoopCycles / loopCycles[i]));
...
bool PositionController::moveToEncoder(int32_t* encoderValues) {
    ...
    if ((this->hardpointActuatorState->motionState[0] != HardpointActuatorMotionStates::Standby &&
         encoderValues[0] != this->hardpointActuatorData->encoder[0]) ||
        (this->hardpointActuatorState->motionState[1] != HardpointActuatorMotionStates::Standby &&
         encoderValues[1] != this->hardpointActuatorData->encoder[1]) ||
        (this->hardpointActuatorState->motionState[2] != HardpointActuatorMotionStates::Standby &&
         encoderValues[2] != this->hardpointActuatorData->encoder[2]) ||
        (this->hardpointActuatorState->motionState[3] != HardpointActuatorMotionStates::Standby &&
         encoderValues[3] != this->hardpointActuatorData->encoder[3]) ||
        (this->hardpointActuatorState->motionState[4] != HardpointActuatorMotionStates::Standby &&
         encoderValues[4] != this->hardpointActuatorData->encoder[4]) ||
        (this->hardpointActuatorState->motionState[5] != HardpointActuatorMotionStates::Standby &&
         encoderValues[5] != this->hardpointActuatorData->encoder[5])) {
        return false;
    }
    this->hardpointActuatorState->timestamp = this->publisher->getTimestamp();
    double loopCycles[6];
    ...
    for (int i = 0; i < HP_COUNT; i++) {
        ...
        this->hardpointActuatorData->stepsQueued[i] =
                deltaEncoder * this->positionControllerSettings->EncoderToStepsCoefficient;
        this->hardpointActuatorState->motionState[i] =
                this->hardpointActuatorData->stepsQueued[i] != 0
                        ? HardpointActuatorMotionStates::QuickPositioning
                        : HardpointActuatorMotionStates::Standby;
        loopCycles[i] = this->abs(this->hardpointActuatorData->stepsQueued[i]) /
                        (double)this->positionControllerSettings->MaxStepsPerLoop;
        if (loopCycles[i] > maxLoopCycles) {
            maxLoopCycles = loopCycles[i];
        }
    }
...
bool PositionController::moveToAbsolute(double x, double y, double z, double rX, double rY, double rZ) {
    ...
    double stepsToEncoder = this->hardpointActuatorSettings->MicrometersPerStep /
    ...
    for (int i = 0; i < HP_COUNT; i++) {
        switch (this->hardpointActuatorState->motionState[i]) {
            ...
            case HardpointActuatorMotionStates::Chasing: {
                ...
                int32_t chaseSteps =
                        (int32_t)(force * this->positionControllerSettings->ForceToStepsCoefficient);
                chaseSteps =
                        Range::CoerceIntoRange(-this->positionControllerSettings->MaxStepsPerLoop,
                                               this->positionControllerSettings->MaxStepsPerLoop, chaseSteps);
                this->hardpointActuatorData->stepsCommanded[i] = (int16_t)chaseSteps;
                break;
            }
            case HardpointActuatorMotionStates::Stepping: {
                int32_t moveSteps = Range::CoerceIntoRange(-this->scaledMaxStepsPerLoop[i],
                                                           this->scaledMaxStepsPerLoop[i],
                                                           this->hardpointActuatorData->stepsQueued[i]);
                this->hardpointActuatorData->stepsQueued[i] -= moveSteps;
                this->hardpointActuatorData->stepsCommanded[i] = (int16_t)moveSteps;
                if (this->hardpointActuatorData->stepsQueued[i] == 0 &&
                    this->hardpointActuatorData->stepsCommanded[i] == 0) {
                    publishState = true;
                    this->hardpointActuatorState->motionState[i] = HardpointActuatorMotionStates::Standby;
                }
            ...
            case HardpointActuatorMotionStates::QuickPositioning: {
                ...
                if (this->hardpointActuatorData->stepsQueued[i] == 0 &&
                    ...
                    publishState = true;
                    this->hardpointActuatorState->motionState[i] =
                            HardpointActuatorMotionStates::FinePositioning;
                }
                break;
            }
            case HardpointActuatorMotionStates::FinePositioning: {
                ...
                int32_t encoderSteps =
                        (int32_t)(deltaEncoder * this->positionControllerSettings->EncoderToStepsCoefficient);
                ...
                this->hardpointActuatorData->stepsCommanded[i] = Range::CoerceIntoRange(
                        -this->positionControllerSettings->MaxStepsPerLoop,
                        this->positionControllerSettings->MaxStepsPerLoop, encoderSteps);
...
void PositionController::convertToSteps(int32_t* steps, double x, double y, double z, double rX, double rY,
                                        double rZ) {
    ...
    steps[2] = (this->hardpointActuatorSettings->MirrorPositionToHardpointDisplacement[0] * x +
               ...
               (MILLIMETERS_PER_METER * MICROMETERS_PER_MILLIMETER /
                this->hardpointActuatorSettings->MicrometersPerStep);
