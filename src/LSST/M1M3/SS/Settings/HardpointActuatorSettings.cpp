...
void HardpointActuatorSettings::load(YAML::Node doc) {
    ...
    inRangeAfterRaiseTimeoutLoops = doc["InRangeAfterRaiseTimeoutLoops"].as<int>(5 * inRangeAfterRaiseLoops);
    ...
    if (inRangeAfterRaiseForce < 0 || inRangeAfterRaiseForce > 200) {
        throw std::runtime_error(fmt::format(
                "InRangeAfterRaiseForce must be positive and less than 200, is {}", inRangeAfterRaiseForce));
    }
