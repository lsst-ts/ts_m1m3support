...
void FPGA::open() {
  ...
  NiOpen("/var/lib/M1M3support", NiFpga_M1M3SupportFPGA, "RIO0", 0,
         &(_session));
  cRIO::NiThrowError(__PRETTY_FUNCTION__, "NiFpga_Abort",
                     NiFpga_Abort(_session));
  cRIO::NiThrowError(__PRETTY_FUNCTION__, "NiFpga_Download",
                     NiFpga_Download(_session));
  cRIO::NiThrowError(__PRETTY_FUNCTION__, "NiFpga_Reset",
                     NiFpga_Reset(_session));
  cRIO::NiThrowError(__PRETTY_FUNCTION__, "NiFpga_Run",
                     NiFpga_Run(_session, 0));
...
void FPGA::close() {
  ...
  cRIO::NiThrowError(__PRETTY_FUNCTION__, "NiFpga_Close",
                     NiFpga_Close(_session, 0));
}
...
void FPGA::waitForOuterLoopClock(uint32_t timeout) {
  ...
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_WaitOnIrqs(_session, _outerLoopIRQContext,
                                       NiFpga_Irq_0, timeout, &assertedIRQs,
                                       &timedOut));
}

void FPGA::ackOuterLoopClock() {
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_AcknowledgeIrqs(_session, NiFpga_Irq_0));
}
...
void FPGA::waitForPPS(uint32_t timeout) {
  ...
  uint8_t timedOut = false;
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_WaitOnIrqs(_session, _ppsIRQContext, NiFpga_Irq_10,
                                       timeout, &assertedIRQs, &timedOut));
}

void FPGA::ackPPS() {
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_AcknowledgeIrqs(_session, NiFpga_Irq_10));
}
...
void FPGA::waitForModbusIRQ(int32_t subnet, uint32_t timeout) {
  ...
  if (irqs != 0) {
    uint32_t assertedIRQs = 0;
    uint8_t timedOut = false;
    cRIO::NiThrowError(NiFpga_WaitOnIrqs(_session, _modbusIRQContext, irqs,
                                         timeout, &assertedIRQs, &timedOut),
...
void FPGA::ackModbusIRQ(int32_t subnet) {
  ...
  if (irqs != 0) {
    cRIO::NiThrowError(__PRETTY_FUNCTION__,
                       NiFpga_AcknowledgeIrqs(_session, irqs));
  }
}
...
void FPGA::pullTelemetry() {
  ...
  supportFPGAData.InclinometerErrorTimestamp =
      U8ArrayUtilities::U64(buffer, 40);
  supportFPGAData.InclinometerErrorCode = U8ArrayUtilities::U8(buffer, 48);
  supportFPGAData.InclinometerSampleTimestamp =
      U8ArrayUtilities::U64(buffer, 49);
  ...
  supportFPGAData.DisplacementErrorTimestamp =
      U8ArrayUtilities::U64(buffer, 93);
  supportFPGAData.DisplacementErrorCode = U8ArrayUtilities::U8(buffer, 101);
  supportFPGAData.DisplacementSampleTimestamp =
      U8ArrayUtilities::U64(buffer, 102);
  ...
  supportFPGAData.AccelerometerSampleTimestamp =
      U8ArrayUtilities::U64(buffer, 150);
  for (int i = 0; i < 8; i++) {
    supportFPGAData.AccelerometerRaw[i] =
        U8ArrayUtilities::SGL(buffer, 158 + i * 4);
  }
...
void FPGA::writeCommandFIFO(uint16_t *data, size_t length,
                            uint32_t timeoutInMs) {
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_WriteFifoU16(
                         _session,
                         NiFpga_M1M3SupportFPGA_HostToTargetFifoU16_CommandFIFO,
                         data, length, timeoutInMs, &_remaining));
}

void FPGA::writeRequestFIFO(uint16_t *data, size_t length,
                            uint32_t timeoutInMs) {
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_WriteFifoU16(
                         _session,
                         NiFpga_M1M3SupportFPGA_HostToTargetFifoU16_RequestFIFO,
                         data, length, timeoutInMs, &_remaining));
}
...
void FPGA::readU8ResponseFIFO(uint8_t *data, size_t length,
                              uint32_t timeoutInMs) {
  cRIO::NiThrowError(
      __PRETTY_FUNCTION__,
      NiFpga_ReadFifoU8(
          _session, NiFpga_M1M3SupportFPGA_TargetToHostFifoU8_U8ResponseFIFO,
          data, length, timeoutInMs, &_remaining));
}

void FPGA::readU16ResponseFIFO(uint16_t *data, size_t length,
                               uint32_t timeoutInMs) {
  cRIO::NiThrowError(
      __PRETTY_FUNCTION__,
      NiFpga_ReadFifoU16(
          _session, NiFpga_M1M3SupportFPGA_TargetToHostFifoU16_U16ResponseFIFO,
          data, length, timeoutInMs, &_remaining));
}

void FPGA::waitOnIrqs(uint32_t irqs, uint32_t timeout, bool &timedout,
                      uint32_t *triggered) {
  ...
  cRIO::NiThrowError(__PRETTY_FUNCTION__,
                     NiFpga_WaitOnIrqs(_session, contex, irqs, timeout,
                                       triggered, &ni_timedout));
...
void FPGA::readHealthAndStatusFIFO(uint64_t *data, size_t length,
                                   uint32_t timeoutInMs) {
  cRIO::NiThrowError(
      __PRETTY_FUNCTION__,
      NiFpga_ReadFifoU64(
          _session,
          NiFpga_M1M3SupportFPGA_TargetToHostFifoU64_HealthAndStatusDataFIFO,
          data, length, timeoutInMs, &_remaining));
}
...
void FPGA::readRawAccelerometerFIFO(uint64_t *raw, size_t samples) {
  ...
  cRIO::NiThrowError(
      ...
      NiFpga_ReadFifoU64(
          _session,
          NiFpga_M1M3SupportFPGA_TargetToHostFifoFxp_RawAccelerometer_Resource,
