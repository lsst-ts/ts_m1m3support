...
ILCResponseParser::ILCResponseParser(ForceActuatorSettings* forceActuatorSettings,
                                     ...
                                     M1M3SSPublisher* publisher, ILCSubnetData* subnetData,
                    ...
                    switch (map.Type) {
                        case ILCTypes::FA:
                            ...
                            switch (function) {
                                ...
                                case 248:
                                case 249:
                                    this->parseErrorResponse(buffer, timestamp, map.ActuatorId);
                                    break;
                                default:
                                    spdlog::warn(
...
void ILCResponseParser::grabNextResponse() { this->grabResponse = true; }
...
void ILCResponseParser::parseOneOffCommand(ModbusBuffer* buffer, uint16_t length, double timestamp,
                                           bool valid) {
    this->grabResponse = false;
...
void ILCResponseParser::parseStepMotorResponse(ModbusBuffer* buffer, ILCMap map, double timestamp) {
    ...
    this->hardpointActuatorWarning->broadcastCounterWarning[dataIndex] =
            this->outerLoopData->broadcastCounter == ((status & 0xF0) >> 4);
    ...
    this->hardpointActuatorData->displacement[dataIndex] =
            (this->hardpointActuatorData->encoder[dataIndex] *
...
void ILCResponseParser::parseSingleAxisForceDemandResponse(ModbusBuffer* buffer, ILCMap map) {
    ...
    // 0x04 is reserved
    // 0x08 is reserved
    this->forceActuatorWarning->broadcastCounterWarning[dataIndex] =
            this->outerLoopData->broadcastCounter != ((status & 0xF0) >> 4);
    this->forceActuatorData->primaryCylinderForce[dataIndex] = buffer->readSGL();
    float x = 0;
    float y = 0;
    float z = 0;
    ForceConverter::saaToMirror(this->forceActuatorData->primaryCylinderForce[dataIndex],
                                this->forceActuatorData->secondaryCylinderForce[dataIndex], &x, &y, &z);
    ...
    switch (this->forceActuatorInfo->actuatorOrientation[dataIndex]) {
        case ForceActuatorOrientations::PositiveX:
            ForceConverter::daaPositiveXToMirror(
                    this->forceActuatorData->primaryCylinderForce[dataIndex],
                    this->forceActuatorData->secondaryCylinderForce[secondaryDataIndex], &x, &y, &z);
            break;
...
void ILCResponseParser::parseHPResetResponse(ModbusBuffer* buffer, ILCMap map) { buffer->skipToNextFrame(); }

void ILCResponseParser::parseFAResetResponse(ModbusBuffer* buffer, ILCMap map) { buffer->skipToNextFrame(); }

void ILCResponseParser::parseHMResetResponse(ModbusBuffer* buffer, ILCMap map) { buffer->skipToNextFrame(); }
...
void ILCResponseParser::parseReadFACalibrationResponse(ModbusBuffer* buffer, ILCMap map) {
    ...
    this->forceActuatorInfo->mainSecondaryCylinderCoefficient[dataIndex] =
            this->forceActuatorInfo->mainPrimaryCylinderCoefficient[dataIndex];
    buffer->readSGL();  // Main Coefficient K2
    ...
    this->forceActuatorInfo->backupSecondaryCylinderCoefficient[dataIndex] =
            this->forceActuatorInfo->backupPrimaryCylinderCoefficient[dataIndex];
    buffer->readSGL();  // Backup Coefficient K2
...
void ILCResponseParser::checkForceActuatorMeasuredForce(ILCMap map) {
    ...
    float primaryLowLimit =
            this->forceActuatorSettings->MeasuredPrimaryCylinderLimitTable[dataIndex].LowFault;
    float primaryHighLimit =
            this->forceActuatorSettings->MeasuredPrimaryCylinderLimitTable[dataIndex].HighFault;
    ...
    if (secondaryDataIndex != -1) {
        ...
        float secondaryLowLimit =
                this->forceActuatorSettings->MeasuredSecondaryCylinderLimitTable[secondaryDataIndex].LowFault;
        float secondaryHighLimit =
                this->forceActuatorSettings->MeasuredSecondaryCylinderLimitTable[secondaryDataIndex]
                        .HighFault;
...
void ILCResponseParser::checkForceActuatorFollowingError(ILCMap map) {
    ...
    float primaryLimit =
            this->forceActuatorSettings->FollowingErrorPrimaryCylinderLimitTable[dataIndex].HighFault;
    ...
    if (secondaryDataIndex != -1) {
        ...
        float secondarySetpoint =
                this->appliedCylinderForces->secondaryCylinderForces[secondaryDataIndex] / 1000.0;
        float secondaryLimit =
                this->forceActuatorSettings->FollowingErrorSecondaryCylinderLimitTable[secondaryDataIndex]
                        .HighFault;
    ...
    this->safetyController->forceActuatorFollowingError(dataIndex,
...
void ILCResponseParser::checkHardpointActuatorMeasuredForce(int32_t actuatorId) {
    ...
    if (this->detailedState->detailedState == MTM1M3::MTM1M3_shared_DetailedStates_ActiveEngineeringState ||
        this->detailedState->detailedState == MTM1M3::MTM1M3_shared_DetailedStates_ActiveState) {
...
void ILCResponseParser::publishForceActuatorForceWarning() {
    ...
    for (int i = 0; i < FA_COUNT; ++i) {
        this->forceWarning->anyPrimaryAxisMeasuredForceWarning =
                this->forceWarning->anyPrimaryAxisMeasuredForceWarning ||
                this->forceWarning->primaryAxisMeasuredForceWarning[i];
        this->forceWarning->anySecondaryAxisMeasuredForceWarning =
                this->forceWarning->anySecondaryAxisMeasuredForceWarning ||
                this->forceWarning->secondaryAxisMeasuredForceWarning[i];
        this->forceWarning->anyPrimaryAxisFollowingErrorWarning =
                this->forceWarning->anyPrimaryAxisFollowingErrorWarning ||
                this->forceWarning->primaryAxisFollowingErrorWarning[i];
        this->forceWarning->anySecondaryAxisFollowingErrorWarning =
                this->forceWarning->anySecondaryAxisFollowingErrorWarning ||
                this->forceWarning->secondaryAxisFollowingErrorWarning[i];
    }
    this->forceWarning->anyWarning = this->forceWarning->anyPrimaryAxisMeasuredForceWarning ||
                                     this->forceWarning->anySecondaryAxisMeasuredForceWarning ||
                                     this->forceWarning->anyPrimaryAxisFollowingErrorWarning ||
                                     this->forceWarning->anySecondaryAxisFollowingErrorWarning;
