...
States::Type StandbyState::start(StartCommand *command) {
  ...
  SettingReader::instance()
      .getSafetyControllerSettings()
      ->ForceController.exitBumpTesting();
  ...
  auto configurationApplied =
      M1M3SSPublisher::instance().getEventConfigurationApplied();
  ...
  configurationApplied->otherInfo = "accelerometerSettings,"
  ...
  for (int i = 0; i < 2; i++) {
    ...
    std::this_thread::sleep_for(
        500ms); // wait 2*0.5=1 second for ILCs to power on
  }
  heartbeat.tryToggle();

// ILC commands issued during startup can take a bit longer than standard loop commands
constexpr int32_t ILC_STARTUP_WAIT = 400;

  ilc->flushAll();
  ilc->writeSetModeClearFaultsBuffer();
  ilc->triggerModbus();
  ilc->waitForAllSubnets(ILC_STARTUP_WAIT);
  ilc->readAll();
  heartbeat.tryToggle();
